import React , { useContext }from 'react'
import OtherUserContext from "../../context/OtherUserContext";

export default function Profile(props) {
    
    
    const { otherUserData } = useContext(OtherUserContext);
    
    
    
    return (
        <div>
        {otherUserData.user.kullaniciYok ? (
            <>
            <h1>Kullanici Yok</h1>
            </>
        ):(
        <>
        <div className="container" pb-3>
        <div className="row">
            
        <div className="col-md-6">
        <img src={otherUserData.user.resim} alt="Logo" style={{height:400,width:400}} />
    </div>
    <div className="col-md-6">
        
    <div style={{textAlign:'center'}}>
                <ul className="list-group-flush" style={{textAlign:'center'}}>
                    <li className="list-group-item">
                        Ad:<h5> {otherUserData.user.isim}</h5>
                    </li>
                    <li className="list-group-item">
                        E-Posta Adresi:<h5>{otherUserData.user.mail}</h5>
                    </li>
                    <li className="list-group-item">
                        Doğum Tarihi:<h5> {otherUserData.user.dogumTarihi}</h5>
                    </li>
                    <li className="list-group-item">
                        İlgilenilen Alanlar:<h5> {otherUserData.user.ilgilenilenAlanlar}</h5>
                    </li>
                    
                </ul>
            </div>
        

        <div className="row mt-3">
            <div className="col-md-12">
                <div className="bd-example bd-example-tabs">
                    <ul className="nav nav-tabs mb-3" id="pills-tab" role="tablist">
                        <li className="nav-item">
                            <a className="nav-link active" id="pills-description-tab" data-toggle="pill" href="#pills-description" role="tab" aria-controls="pills-description" aria-expanded="true">Hakkımda</a>
                        </li>

                    </ul>
                    <div className="tab-content" id="pills-tabContent">
                        <div className="tab-pane fade show active" id="pills-description" role="tabpanel" aria-labelledby="pills-description-tab">
                            <p>{otherUserData.user.hakkimda}</p>
                        </div>

                    </div>
                </div>
            </div>
        </div>
        <div className="row mt-2" style={{float:'right'}}>
            <div className="col-md-12">

                
                
                
            </div>
        </div>

    </div>
    
    
   
</div>

</div>
        </>
        )}
    </div>
        

    );
}